...
def generate_payment_amount(distribution='exponential', min_amount=1, max_amount=10000, **kwargs):
    """
...
class TestAdditionalPaymentMethods:
    ...
    @pytest.mark.parametrize("invalid_amount", [-100, 0, 'abc', 10.5, None])
    @patch.object(PaymentProcessor, 'process_payment')
    def test_invalid_payment_amounts(self, mock_process_payment, invalid_amount):
        logger.info(f"Testing invalid payment amount: {invalid_amount}")
        with pytest.raises(ValueError):
            validate_payment_amount(invalid_amount, min_amount=1, max_amount=10000)
